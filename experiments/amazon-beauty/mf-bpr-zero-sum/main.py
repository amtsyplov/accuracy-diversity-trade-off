...
class BPRWithZeroSumLoss(nn.Module):
    ...
    def forward(
        ...
        loss = -self.log_sigmoid(positive_score - negative_score)
        reg = (positive_score + negative_score) ** 2
        return torch.mean(loss + self.alpha * reg)
...
def main(filepath: str) -> None:
    ...
    with mlflow.start_run():
        ...
        loss_fn = BPRWithZeroSumLoss(alpha=config["alpha"])
